...
export class QueueManager {
    ...
    public async loadQueue(queueId: string, player: LithiumXPlayer, options: LoadQueueOptions = {}): Promise<QueueOperationResult> {
        try {
            ...
            if (!savedQueue || !savedQueue.tracks || !Array.isArray(savedQueue.tracks)) {
                return {
                    success: false,
    ...
    public async listQueues(guildId: string, includeGlobal = true): Promise<SavedQueue[]> {
        try {
            ...
            try {
                ...
                const guildQueues = guildQueueIds
                    .filter(id => id.startsWith(`${guildId}/`))
                    .map(id => id.substring(`${guildId}/`.length));
            ...
            if (includeGlobal) {
                try {
                    ...
                    const globalQueues = allQueueIds
                        .filter(id => id.startsWith('global/'))
                        .map(id => id.substring('global/'.length));
