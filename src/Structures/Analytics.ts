...
export class Analytics {
	...
	private registerEventListeners(): void {
		...
		this.manager.on('QueueEnd', (player, _track) => {
			if (!this.options.enabled) return;
	...
	private trackPlay(guildId: string, track: Track): void {
		...
		if (this.options.trackUsers && track.requester) {
			const userId = this.options.anonymizeUsers ? this.anonymizeUser(track.requester) : track.requester;
	...
	private updatePlayStats(guildId: string, track: Track, duration: number, percentage: number, skipped: boolean): void {
		...
		analytics.artists.playTime[artistName] = (analytics.artists.playTime[artistName] || 0) + duration;
	...
	private endSession(guildId: string): void {
		...
		analytics.sessions.averageSessionDuration = (oldAvg * (totalSessions - 1) + sessionDuration) / totalSessions;
	 ...
	 * @param guildId Guild ID
	 */
	private getTotalSessionCount(_guildId: string): number {
	...
	private async loadData(): Promise<void> {
		try {
			...
			const guildKeys = await this.storage.getAll().then((keys) => keys.filter((key) => key.startsWith('guilds/')));
	...
	public getUserRecommendations(guildId: string, userId: string, limit = 5): string[] {
		...
		const userIdKey = this.options.anonymizeUsers ? this.anonymizeUser(userId) : userId;
		...
		Object.entries(analytics.tracks.playCount).forEach(([trackId, _stats]) => {
	...
	public getSummaryStats(guildId: string): {
		...
		Object.entries(analytics.sessions.peakTimes).forEach(([hour, count]) => {
			if (count > maxCount) {
				maxCount = count;
				mostPopularHour = parseInt(hour, 10);
			}
		});
		...
		return {
			...
			totalPlayTime: Object.values(analytics.tracks.playCount).reduce((sum, stats) => sum + stats.totalPlayTime, 0),
